...
from app.export_service import build_dxf_bytes, build_stl_bytes
...
class OutlineExportRequest(BaseModel):
    name: str = Field("gearcalc_export", min_length=1, max_length=120)
    x_mm: list[float] = Field(..., min_length=3)
    y_mm: list[float] = Field(..., min_length=3)
...
@app.post("/export_dxf")
def export_dxf(payload: OutlineExportRequest) -> Response:
    if len(payload.x_mm) != len(payload.y_mm):
        raise HTTPException(status_code=400, detail="x_mm and y_mm must have the same length.")
    try:
        data = build_dxf_bytes(payload.name, payload.x_mm, payload.y_mm)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"DXF export failed: {exc}") from exc
    filename = f"{payload.name}.dxf"
    return Response(
        content=data,
...
@app.post("/export_stl")
def export_stl(payload: OutlineExportRequest) -> Response:
    if len(payload.x_mm) != len(payload.y_mm):
        raise HTTPException(status_code=400, detail="x_mm and y_mm must have the same length.")
    try:
        data = build_stl_bytes(payload.name, payload.x_mm, payload.y_mm, payload.thickness_mm)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"STL export failed: {exc}") from exc
    filename = f"{payload.name}.stl"
    return Response(
        content=data,
