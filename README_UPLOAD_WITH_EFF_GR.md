# GEARCALC PRO READY PACKAGE (Efficiency)
...
- `index.html`
  - Home, Geometry, Strength (AGMA+ISO), STE (NN), Efficiency, Theory, Exports
- `app/main.py`
  - API routes: `GET /`, `GET /healthz`, `POST /predict_ste`, `POST /predict_efficiency`, `POST /compute_geometry`
- `app/ste_service.py`
...
git init
git add .
git commit -m "GEARCALC PRO: Efficiency update"
git branch -M main
